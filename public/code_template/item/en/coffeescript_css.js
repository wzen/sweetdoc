...
Temp = (function(superClass) {
  extend(Temp, superClass);

  function Temp() {
    return Temp.__super__.constructor.apply(this, arguments);
  }

  Temp.IDENTITY = "Temp";

  if (window.loadedItemId != null) {
    Temp.ITEM_ID = window.loadedItemId;
  }

  Temp.actionProperties = {
  ...
  Temp.prototype.reDraw = function(show) {
    if (show == null) {
      show = true;
    }
    return Temp.__super__.reDraw.call(this, show);
  };

  Temp.prototype.updateEventBefore = function() {
    var methodName;
    Temp.__super__.updateEventBefore.call(this);
  ...
  Temp.prototype.updateEventAfter = function() {
    var methodName;
    Temp.__super__.updateEventAfter.call(this);
  ...
  Temp.prototype.defaultClick = function(opt) {
  ...
  Temp.prototype.changeColorScroll = function(opt) {
  ...
  Temp.prototype.changeColorClick = function(opt) {
    this.getJQueryElement().find('.css3button').css('background', "#" + this.backgroundColor);
    if (opt.complete != null) {
      return opt.complete();
    }
  ...
  Temp.prototype.cssAnimationKeyframe = function() {
  ...
  Temp.prototype.willChapter = function() {
    Temp.__super__.willChapter.call(this);
...
if ((window.itemInitFuncList != null) && (window.itemInitFuncList[Temp.ITEM_ID] == null)) {
  console.log('temp loaded');
  window.itemInitFuncList[Temp.ITEM_ID] = function(option) {
    if (option == null) {
      option = {};
    }
    if (window.isWorkTable && (Temp.jsLoaded != null)) {
      Temp.jsLoaded(option);
    }
    if (window.debug) {
      return console.log('temp init finished');
    }
  };
}
