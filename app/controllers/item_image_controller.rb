...
class ItemImageController < ApplicationController
  ...
  def create_img
    ...
    @result_success, @message, @image_url = create_item_image(user_id, project_id, item_obj_id, event_dist_id, file_path, url)
  end

  def remove_worktable_project_img
    user_id = current_or_guest_user.id
    project_id = params.require(Const::PreloadItemImage::Key::PROJECT_ID)
    @result_success, @message = remove_worktable_item_image(user_id, project_id)
    render 'ret'
  end
  ...
  def remove_worktable_item_img
    ...
    project_id = params.require(Const::PreloadItemImage::Key::PROJECT_ID)
    item_obj_id = params.require(Const::PreloadItemImage::Key::ITEM_OBJ_ID)
    @result_success, @message = remove_worktable_item_image(user_id, project_id, item_obj_id)
    render 'ret'
  end
  ...
  def remove_gallery_img
    ...
    @result_success, @message = remove_gallery_item_image(user_id, gallery_id)
    render 'ret'
  end
end
