...
class RunController < ApplicationController
  ...
  def paging
    ...
    loaded_item_access_tokens = params.require(Const::Run::Key::LOADED_ITEM_ACCESS_TOKENS)
    if loaded_item_access_tokens.present?
      loaded_item_access_tokens = JSON.parse(loaded_item_access_tokens)
    end
    if Rails.application.routes.recognize_path(request.referer)[:controller] == "gallery"
      ...
      @result_success, @pagevalues, @item_js_list = Gallery.paging(user_id, access_token, target_pages, loaded_item_access_tokens, load_footprint)
    else
      user_id = current_or_guest_user.id
      user_pagevalue_id = params.require(Const::Run::Key::RUNNING_USER_PAGEVALUE_ID)
      @result_success, @pagevalues, @item_js_list = MotionCheck.paging(user_id, user_pagevalue_id, target_pages, loaded_item_access_tokens)
    end
  end
