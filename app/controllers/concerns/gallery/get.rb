module GalleryGet
  ...
  def _get_project_pagevalues(upm_id)
  ...
  def firstload_contents(user_id, access_token, hostname, page_num = 1)
    ...
    if ret.count == 0
      ...
      string_link = string_link(access_token, hostname, ret[Const::Gallery::Key::TITLE])
      embed_link = embed_link(gpd, access_token, hostname)
  ...
  def string_link(access_token, hostname, title)
  ...
  def embed_link(gpd, access_token, hostname)
  ...
  def get_creator_info_by_gallery_id(gallery_id)
    begin
      sql =<<-"SQL"
  ...
  def created_contents_list(user_id, head = 0, limit = 30)
    sql =<<-"SQL"
      SELECT
  ...
  def bookmarks_list(user_id, head = 0, limit = 30)
    sql =<<-"SQL"
      SELECT
        g.access_token as #{Const::Gallery::Key::GALLERY_ACCESS_TOKEN},
        g.title as #{Const::Gallery::Key::TITLE},
  ...
  def paging(user_id, access_token, target_pages, loaded_class_dist_tokens, load_footprint)
  ...
  def get_contents_with_tags(user_id, access_token)
    if user_id
      sql =<<-"SQL"
      SELECT
        g.title as #{Const::Gallery::Key::TITLE},
  ...
  def _load_viewcount_and_bookmarkcount(gallery_id)
