module GallerySave
  ...
  def _save_gallery_pagevalue(pagevalues, gallery_id)
  ...
  def add_view_statistic_count(access_token, date)
    begin
  ...
  def _add_bookmark(user_id, gallery_access_token, note, date)
    begin
      ActiveRecord::Base.transaction do
  ...
  def _remove_bookmark(user_id, gallery_access_token, date)
    begin
      ActiveRecord::Base.transaction do
        g = Gallery.find_by({access_token: gallery_access_token, del_flg: false})
        if g.present?
  ...
  def _update_last_state(user_id, tags, i_page_values, e_page_values)
    begin
