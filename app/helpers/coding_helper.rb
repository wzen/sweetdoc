module CodingHelper
  ...
  def code_parentdirpath(type, user_access_token)
  ...
  def code_filepath(type, user_access_token, code_filename)
    code_parentdirpath(type, user_access_token) + "/#{code_filename}"
  end

  def code_urlpath(type, user_access_token, code_filename)
    if type == CODE_TYPE::ITEM_PREVIEW
  ...
  def update_code(type, user_id, codes)
    ret = []
    user_access_token = User.find(user_id)['access_token']
    FileUtils.mkdir_p(code_parentdirpath(type, user_access_token)) unless File.directory?(code_parentdirpath(type, user_access_token))
    codes.each do |c|
  ...
  def save_code(type, user_access_token, code_filename, lang_type, code)
