...
Common = (function() {
  ...
  Common.colorChangeCacheData = function(beforeColor, afterColor, length, colorType) {
    var aa, ab, b, bColors, bPer, bp, cColors, g, gPer, gp, i, index, j, len, len1, len2, len3, o, q, r, rPer, ref, ref1, ret, rp, u, val, z;
    if (colorType == null) {
      colorType = 'hex';
    }
    ret = [];
    ...
    if (afterColor.length === 6 || (afterColor.length === 7 && afterColor.indexOf('#') === 0)) {
      ...
      for (index = q = 0, len1 = cColors.length; q < len1; index = ++q) {
        val = cColors[index];
        cColors[index] = parseInt(val, 16);
      }
    }
    if (beforeColor === 'transparent') {
      for (i = u = 0, ref = length; 0 <= ref ? u <= ref : u >= ref; i = 0 <= ref ? ++u : --u) {
      ...
      if (beforeColor.indexOf('rgb') >= 0) {
        ...
        for (index = z = 0, len2 = bColors.length; z < len2; index = ++z) {
          val = bColors[index];
      ...
      if (beforeColor.length === 6 || (beforeColor.length === 7 && beforeColor.indexOf('#') === 0)) {
        ...
        for (index = aa = 0, len3 = bColors.length; aa < len3; index = ++aa) {
          val = bColors[index];
      ...
      gp = gPer;
      bp = bPer;
      for (i = ab = 0, ref1 = length; 0 <= ref1 ? ab <= ref1 : ab >= ref1; i = 0 <= ref1 ? ++ab : --ab) {
  ...
  Common.markdownToHtml = function() {
    var md;
    md = $('.markdown');
